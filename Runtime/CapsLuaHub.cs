    ...
    public static partial class LuaHub
    {
        ...
        public static void PushLua(this IntPtr l, object val)
        {
            if (l != IntPtr.Zero)
            {
                ...
                else if (val is LuaWrap.BaseLua)
                {
        ...
        {
            bool isvalue = typeof(T).IsValueType;
...
#region Push Methods for Native Lua Types
                    ...
                    case lua.LUA_TTABLE:
                        {
                            ...
                            return isUserData ? obj : new Capstones.LuaWrap.LuaTable(l, index);
                        }
                    case lua.LUA_TNUMBER:
            ...
            if (l != IntPtr.Zero)
            {
                ...
                switch (typecode)
                {
                    ...
                    case lua.LUA_TTABLE:
                        {
                            bool isUserData;
                            var obj = GetLuaTableObject(l, index, out isUserData);
                            return isUserData ? obj : new Capstones.LuaWrap.LuaOnStackTable(l, index);
                        }
                    case lua.LUA_TNUMBER:
                        return l.tonumber(index);
                    case lua.LUA_TBOOLEAN:
                    ...
                    case lua.LUA_TLIGHTUSERDATA:
                        return l.touserdata(index);
                    case lua.LUA_TFUNCTION:
                        return new Capstones.LuaWrap.LuaOnStackFunc(l, index);
                    case lua.LUA_TTHREAD:
                        IntPtr lthd = l.tothread(index);
                        l.pushvalue(index);
                        int refid = l.refer();
        ...
        public static void GetLuaExplicit<T>(this IntPtr l, int index, out T val)
        {
            if (l != IntPtr.Zero)
            {
                ...
                else if (typeof(T) == typeof(Capstones.LuaWrap.LuaFunc))
                {
                    if (l.isfunction(index))
                    {
                        val = (T)(object)new Capstones.LuaWrap.LuaFunc(l, index);
                        return;
                    }
                }
                else if (typeof(T) == typeof(Capstones.LuaWrap.LuaOnStackFunc))
                {
                    if (l.isfunction(index))
                    {
                        val = (T)(object)new Capstones.LuaWrap.LuaOnStackFunc(l, index);
                        return;
                    }
                }
