...
#ifndef CAPSLUA_USE_ORIGINAL_LJ_RANDOM
...
#define CAPS_RAND_MAX 2147483647
#endif
...
LJLIB_PUSH(top-2)  /* Upvalue holds userdata with RandomState. */
LJLIB_CF(math_randomcaps)
{
#ifdef CAPSLUA_USE_ORIGINAL_LJ_RANDOM
  return 0;
#else
  int n = (int)(L->top - L->base);
  RandomState2 *rs = (RandomState2 *)(uddata(udataV(lj_lib_upvalue(L, 1))));
  if (LJ_UNLIKELY(!rs->valid)) redisSrand48(rs, 0);
  double d = (double)(redisLrand48(rs) % CAPS_RAND_MAX) / (double)CAPS_RAND_MAX;
  if (n > 0) {
#if LJ_DUALNUM
    int isint = 1;
    double r1;
